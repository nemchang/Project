...
class MainGUI:
    ...
    def InputSearchTab(self):
        ...
        # 시/군 입력
        self.SearchLabel = Label(self.frame_SearchTab, text=" 시/군 입력 ", font=("한수원 한돋움", 13, 'bold'), fg="#ffffff",
                                 bg='#005CB2')
        self.SearchLabel.place(x=5, y=12)
        # 입력창
        self.searchPlaceInput = StringVar()
        self.SearchEntry = Entry(self.frame_SearchTab, textvariable=self.searchPlaceInput, bg='#F0F0F0',
                                 font=("한수원 한돋움", 14, 'bold'), relief='ridge')
        self.SearchEntry.place(x=100, y=12)
        # 검색 버튼
        self.SearchButton = Button(self.frame_SearchTab, text="검색!", command=self.SearchButtonAtion, relief="ridge",
                                   font=("한수원 한돋움", 13, 'bold'),
    ...
    def InputSaveTab(self):
        ...
        self.telegramButton = Button(self.frame_SaveTab, image=self.image_telegram, command=self.sendTelegram,
                                     bg='#005CB2')
    ...
    def SearchFranchise(self):
        ...
        # 가맹점리스트 먼저 생성
        self.ListBox = Listbox(self.frame_SearchTab, width=33, height=15, borderwidth=3, relief='ridge',
                               activestyle=DOTBOX,
        ...
        conn.request("GET",
                     "/RegionMnyFacltStus?KEY=a5ff90a0a64c48ee83f8ff3250b31afd&pIndex=3&pSize=20&SIGUN_NM=" + hangul_utf8,
        ...
        # 선택한 가게 정보 보기 버튼 삽입
        self.ShowButton = Button(self.frame_SearchTab, text="GET INFO", command=self.ShowInfo,
                                 font=("한수원 한돋움", 11, 'bold'),
                                 activeforeground="#F0F0F0", activebackground='#005CB2', bg='#ffffff', fg="#005CB2",
                                 relief='ridge', height=1, width=43)
    ...
    def ShowInfo(self):
        ...
        # 가게이름, 주소 , 우편번호 출력
        self.selecLabel_NM = Label(self.frame_SearchTab, text=self.selection, font=("한수원 한돋움", 20), bg="#ffffff",
                                   fg='#005CB2')
        self.selecLabel_NM.place(x=495, y=50)
        if DataList[self.ListBox.index(self.ListBox.curselection())][1] == None:
            self.selecLabel_LA = Label(self.frame_SearchTab, text="주소 : - ", font=("한수원 한돋움", 9), bg="#ffffff",
                                       fg='#005CB2')
        else:
            self.selecLabel_LA = Label(self.frame_SearchTab,
                                       text=DataList[self.ListBox.index(self.ListBox.curselection())][1],
                                       font=("한수원 한돋움", 9, "underline"), bg="#ffffff", fg='#005CB2')
        self.selecLabel_LA.place(x=460, y=85)
        if DataList[self.ListBox.index(self.ListBox.curselection())][2] == None:
            self.selecLabel_ZC = Label(self.frame_SearchTab, text="우편번호: - ", font=("한수원 한돋움", 10), bg="#ffffff",
                                       fg='#005CB2')
        else:
            self.selecLabel_ZC = Label(self.frame_SearchTab,
                                       text="우편번호: " + DataList[self.ListBox.index(self.ListBox.curselection())][2],
                                       font=("한수원 한돋움", 10), bg="#ffffff", fg='#005CB2')
        self.selecLabel_ZC.place(x=460, y=105)

        # 길찾기 로드뷰 버튼 생성
        self.bb_image = PhotoImage(file="image/roadsign.png")
        self.myplace2selec = Button(self.frame_SearchTab, image=self.bb_image, command=lambda: self.Showkakao(2),
                                    bg="#ffffff")
        self.myplace2selec.place(x=795, y=200)
        self.cc_image = PhotoImage(file="image/roadview.png")
        self.selecroadview = Button(self.frame_SearchTab, image=self.cc_image, command=lambda: self.Showkakao(3),
                                    bg="#ffffff")
        self.selecroadview.place(x=795, y=260)

        # 가맹점 저장 버튼 생성
        self.save = PhotoImage(file="image/save3.png")
        self.SaveFranchiseButton = Button(self.frame_SearchTab, image=self.save, bg='#ffffff', borderwidth=0,
                                          relief="flat", command=self.SaveFranchise)
    ...
    def SaveFranchise(self):
        ...
        self.save_ListLabel = Label(self.frame_SaveTab, text=" 저장한 가맹점 리스트 ", font=("한수원 한돋움", 15, 'bold'),
                                    fg="#ffffff", bg='#005CB2')
